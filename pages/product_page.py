...
class ProductPage(BasePage):
    ...
    def user_add_to_basket(self):
        ...
        button_add_to_basket = self.browser.find_element(*Locators.ADD_TO_BASKET_BUTTON)
    ...
    def should_be_message_about_adding(self):
        ...
        assert self.is_element_present(*Locators.BOOK_TITLE), "Product name is not presented"
        assert self.is_element_present(*Locators.ADDED_BOOK_TITLE), "Message about adding is not presented"
        # Затем получаем текст элементов для проверки
        product_name = self.browser.find_element(*Locators.BOOK_TITLE).text
    ...
    def should_be_message_basket_total(self):
        assert self.is_element_present(*Locators.BASKET_TOTAL), "Message basket total is not presented"
        assert self.is_element_present(*Locators.BOOK_PRICE), "Product price is not presented"
        # Затем получаем текст элементов для проверки
        message_basket_total = self.browser.find_element(*Locators.BASKET_TOTAL).text
        product_price = self.browser.find_element(*Locators.BOOK_PRICE).text
        # Проверяем, что цена товара присутствует в сообщении со стоимостью корзины
        assert product_price == message_basket_total, "Price is wrong in the message"
        #assert product_price in message_basket_total, "No product price in the message"

    def should_not_be_success_message(self):
        assert self.is_not_element_present(*Locators.ADDED_BOOK_TITLE), \
            "Success message is presented, but should not be"

    def element_disappeared(self):
        assert self.is_disappeared(*Locators.ADDED_BOOK_TITLE), \
